...
import { applyDiscount, fetchBookingOffers, fetchBookingMessages, type OfferLogItem, type AdminMessage } from "@/lib/admin-api";
...
// ─── Conversation ──────────────────────────────────────────
// The full customer <-> cleaner thread for this booking. Read-only: this is
// the record you need when someone disputes what was or wasn't agreed.
function Conversation({ bookingId }: { bookingId: string }) {
  ...
  useEffect(() => {
    ...
    fetchBookingMessages(bookingId)
      .then((r) => { if (!gone) setMsgs(r.data.messages); })
      .catch((e) => { if (!gone) setErr(e instanceof Error ? e.message : "Couldn't load messages"); });
    return () => { gone = true; };
  }, [bookingId]);
  ...
  return (
    <div style={{ background: "white", border: "1px solid var(--color-rule, #e5eaf2)", borderRadius: 12, padding: "12px 14px", marginBottom: 16 }}>
      ...
      >
        <div style={{ fontSize: 13, fontWeight: 700, color: "#0D1B3E" }}>
          Conversation {msgs ? `(${count} message${count === 1 ? "" : "s"})` : ""} {open ? "▾" : "▸"}
        </div>
      </button>
      ...
      {open && (
        <div style={{ marginTop: 10 }}>
          {err && <div style={{ fontSize: 12, color: "#b91c1c" }}>{err}</div>}
          {!msgs && !err && <div style={{ fontSize: 12, color: "#9ca3af" }}>Loading…</div>}
          {msgs && count === 0 && (
            <div style={{ fontSize: 12, color: "#9ca3af" }}>No messages on this booking.</div>
          )}
          <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 340, overflowY: "auto" }}>
            {msgs?.map((m) => {
              const fromPro = m.sender === "pro";
              return (
                <div key={m.id} style={{ alignSelf: fromPro ? "flex-end" : "flex-start", maxWidth: "80%" }}>
                  <div style={{
                    background: fromPro ? "#eff6ff" : "#f8fafc",
                    border: `1px solid ${fromPro ? "#bfdbfe" : "#e2e8f0"}`,
                    borderRadius: 12, padding: "8px 12px", fontSize: 13, lineHeight: 1.45,
                    color: "#0D1B3E", whiteSpace: "pre-wrap",
                  }}>
                    {m.body}
                  </div>
                  <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 2, textAlign: fromPro ? "right" : "left" }}>
                    {fromPro ? "Cleaner" : m.sender === "admin" ? "BubbleBox" : "Customer"} · {new Date(m.created_at + "Z").toLocaleString()}
                  </div>
                </div>
              );
            })}
